...
class MyString{
    ...
    ~MyString();
    MyString& assign(const MyString& str);  // MyString 타입 문자열 복사
    MyString& assign(const char* str);      // 문자배열 문자열 복사
    MyString& insert(int loc, const MyString& str);
    MyString& insert(int loc, const char* str);
    MyString& insert(int loc, char c);
    MyString& erase(int loc, int num);
    int length() const;                     // 문자열 길이
    void print() const;                     // 출력
    void println() const;                   // 출력 후 줄 넘김
    int capacity();                         // 현재 할당된 메모리 용량 반환
    void reserve(int size);                 // size 만큼의 메모리 용량 확보
    char at(int i) const;
...
MyString& MyString::insert(int loc, const MyString& str){
    ...
    // 예를 들어 abc 라는 문자열에 insert(1, "b") 를 하게 된다면 adbc가 된다.
    ...
    // 효율적으로 insert 하기위해, 밀리는 부분을 먼저 뒤로 밀어버린다.
    for(int i=string_length - 1; i >=loc; i--){
        // 뒤모 밀기. 이 때 원래의 문자열 데이터가 사라지지 않게 함
...
void MyString::reserve(int size){
    ...
    // 만일 예약할려는 size 가 현재 capacity 보다 작다면 아무것도 안해도 된다.
}
...
int MyString::compare(const MyString& str) const{
    // (*this) - (str) 을 수행서 그 1, 0, -1 로 그 결과를 리턴한다
    // 1 은 (*this) 가 사전식으로 더 뒤에 온다는 의미, 0 은 두 문자열이 같다는 의미, -1 은 (*this) 가 사전식으로 더 앞에 온다는 의미이다.
