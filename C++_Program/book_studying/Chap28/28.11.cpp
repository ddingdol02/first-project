...
// 28.12 회의 시간으로부터 2-SAT 문제의 함의 그래프 만들기
...
// mettings[]가 각 팀이 하고 싶어하는 회의 시간의 목록일 때, 이를
...
void makeGraph(const vector<pair<int, int>> &meetings)
{
    ...
    for (int i = 0; i < vars; ++i)
        for (int j = 0; j < i; ++j)
        {
            ...
            {
                // i번 회으기 열리지 않거나, j번 회의가 열리지 않아야 한다.
                // (~i or ~j) 절을 추가한다.
...
vector<int> solve2SAT()
{
    ...
    // 이 SAT 문제를 푸ㅡㄴ 것이 불가능한지 확인한다. 한 변수를 나타내는 두 정점이
